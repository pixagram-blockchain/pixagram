...
namespace hive { namespace protocol {
  ...
  struct legacy_chain_properties
  {
    ...
    void validate()const
    {
      if( force_canon )
      {
        HIVE_PROTOCOL_ASSET_ASSERT( account_creation_fee.symbol.is_canon(), "Account creation fee must be in canonical form", ("subject", account_creation_fee) );
      }
      HIVE_PROTOCOL_ASSET_ASSERT( account_creation_fee.amount >= HIVE_MIN_ACCOUNT_CREATION_FEE, "Account creation fee must be at least the minimum",
        ("subject", account_creation_fee)("min", HIVE_MIN_ACCOUNT_CREATION_FEE)
      );
      HIVE_PROTOCOL_ASSET_ASSERT( account_creation_fee.amount <= HIVE_MAX_ACCOUNT_CREATION_FEE, "Account creation fee cannot be too large",
        ("subject", account_creation_fee)("max", HIVE_MAX_ACCOUNT_CREATION_FEE)
      );
