...
BOOST_AUTO_TEST_CASE( declined_voting_rights_basic )
{
  try
  {
    ...
    auto _content = [&is_hf28]( ptr_hardfork_database_fixture& executor )
    {
        ...
        BOOST_REQUIRE_EQUAL( itr->effective_date, executor->db->head_block_time() + HIVE_OWNER_AUTH_RECOVERY_PERIOD );
        ...
        */
        ...
        BOOST_REQUIRE_EQUAL( recent_op.account, "alice" );
    };
      ...
      }
      {
        ...
        else
        {
          ...
          catch( const fc::assert_exception& ex )
          {
            ...
            BOOST_REQUIRE_NE( ex.to_string().find( "Duplicate transaction check failed" ), std::string::npos );
          }
        ...
        else
        {
          ...
          catch( const fc::assert_exception& ex )
          {
            ...
            if( _additional_statuses[_test_cnt] )
              ...
              BOOST_REQUIRE_NE( ex.to_string().find( "trx.expiration <= now" ), std::string::npos );
          }
          BOOST_REQUIRE( !_passed );
        }
        ++_test_cnt;
...
BOOST_AUTO_TEST_CASE( different_behaviour_for_nonexistent_proposals )
{
  try
  {
    ...
    auto _content = [ &is_hf28 ]( ptr_hardfork_database_fixture& executor )
    {
      ...
      //An exception must be thrown because there is a condition `_db.has_hardfork( HIVE_HARDFORK_1_28 )`
      if( is_hf28 )
      ...
      //An exception must be thrown because there is a condition `_db.has_hardfork( HIVE_HARDFORK_1_28 )`
      if( is_hf28 )
        HIVE_REQUIRE_ASSERT( executor->vote_proposal("alice", { _id_proposal, 667 }, true, alice_private_key), "false && \"proposal not found\"" );
      else
        executor->vote_proposal("alice", { _id_proposal, 667 }, true, alice_private_key);

      BOOST_TEST_MESSAGE( "Create 'remove_proposal_operation'" );
      //An exception must be thrown because there is a condition `_db.has_hardfork( HIVE_HARDFORK_1_28 )`
      if( is_hf28 )
        HIVE_REQUIRE_ASSERT( executor->remove_proposal("alice", { 668 }, alice_private_key), "false && \"proposal doesn't exist\"" );
      else
        executor->remove_proposal("alice", { 668 }, alice_private_key);

      //An exception must be thrown because there is a condition `_db.has_hardfork( HIVE_HARDFORK_1_28 )`
      if( is_hf28 )
        HIVE_REQUIRE_ASSERT( executor->remove_proposal("alice", { _id_proposal, 669 }, alice_private_key), "false && \"proposal doesn't exist\"" );
      else
        executor->remove_proposal("alice", { _id_proposal, 669 }, alice_private_key);
    };
...
void set_witness_properties( database_fixture& fixture, uint32_t block_size, uint16_t hbd_apr, const HIVE_asset& fee )
{
  ...
  props[ "account_creation_fee" ] = fc::raw::pack_to_vector( fee.to_asset() );
  ...
  ilog( "#${b} setting new witness values: maximum_block_size = ${s}, hbd_interest_rate = ${h}, account_creation_fee = ${f}",
    ( "b", fixture.db->head_block_num() )( "s", block_size )( "h", hbd_apr )( "f", fixture.asset_to_string( fee ) ) );
};
...
BOOST_AUTO_TEST_CASE( global_witness_props_basic_test )
{
  try
  {
    ...
    BOOST_REQUIRE_EQUAL( future_wso.median_props.account_creation_fee, default_fee );
    BOOST_REQUIRE_EQUAL( witness.props.account_creation_fee, default_fee );
...
BOOST_AUTO_TEST_CASE( global_witness_props_change_noticed_after_hf_test )
{
  try
  {
    ...
    BOOST_REQUIRE_EQUAL( witness.props.account_creation_fee, new_fee );
