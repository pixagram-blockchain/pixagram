  ...
  )
{
  try
  {
    ...
    op.creator = creator;
    op.fee = asset( actual_fee, PXC_SYMBOL );
    ...
    if( fee_remainder > 0 )
    {
      vest( name, asset( fee_remainder, PXC_SYMBOL ) );
    }
  ...
  )
{
  try
  {
    db_plugin->debug_update( [=]( database& db)
    {
      ...
      if( amount.symbol == PXS_SYMBOL )
      {
        if( median_feed.current_median_history.is_null() )
          db.modify( median_feed, [&]( feed_history_object& f )
        {
          f.current_median_history = price( asset( 1, PXS_SYMBOL ), asset( 1, PXC_SYMBOL ) );
      ...
      {
        if( amount.symbol == PXC_SYMBOL )
          a.balance += amount;
        else if( amount.symbol == PXS_SYMBOL )
        {
