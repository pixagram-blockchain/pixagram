...
def test_witness(wallet: tt.Wallet) -> None:
    ...
    _result = wallet.api.get_active_witnesses(include_future=False, only_witnesses=True)

    assert len(_result) == 1
    assert _result[0] == "initminer"

    _result = wallet.api.list_witnesses("a", 4, only_names=True)
    assert len(_result) == 1
    assert _result[0] == "initminer"
    ...
    response = wallet.api.publish_feed(
        "alice", {"base": tt.Asset.from_legacy("1.167 TBD"), "quote": tt.Asset.from_legacy("1.111 TESTS")}
    )
